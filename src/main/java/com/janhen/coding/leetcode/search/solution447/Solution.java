package com.janhen.coding.leetcode.search.solution447;
...
class Solution {
    ...
    public int numberOfBoomerangs(int[][] points) {
        ...
        for (int i = 0; i < points.length; i ++) {
            Map<Integer, Integer> record = new HashMap<>();   // current point x, every point ↔ record
            for (int j = 0; j < points.length; j ++) {
                if (j != i) {
                    int distance = distance(points[i], points[j]);
                    record.put(distance, record.getOrDefault(distance, 0) + 1);   // collect all distance(x,y) frequency
                }
            }
            // collect result
            // if 1    1 * 0   can  hidden
            for (int key : record.keySet())   // insure x, find {y,z}
                if (record.get(key) >= 2)
                    res += record.get(key) * (record.get(key) - 1);    // C(n,2) = freq*(freq-1)
        }
        return res;
    }
